...
v5 = [0.0009977, 0.0009996, 0.0010057, 0.0010149, 0.0010267, 0.001041,0.0010576, 0.0010769, 0.0010988, 0.001124, 0.0011531, 0.0011868,0.0012268, 0.0012755]
...
for i in range(len(temp5)):
    if temp == temp5[i]:        # if equal to lower bound
        ...
        vh = interpolate(temp10[i], v10[i], temp10[i + 1], v10[i + 1], 
        temp)
        ul = interpolate(temp5[i], u5[i], temp5[i + 1], u5[i + 1], temp)
        uh = interpolate(temp10[i], u10[i], temp10[i + 1], u10[i + 1], 
        temp)
        hl = interpolate(temp5[i], h5[i], temp5[i + 1], h5[i + 1], temp)
        hh = interpolate(temp10[i], h10[i], temp10[i + 1], h10[i + 1], 
        temp)
        sl = interpolate(temp5[i], s5[i], temp5[i + 1], s5[i + 1], temp)
        sh = interpolate(temp10[i], s10[i], temp10[i + 1], s10[i + 1], 
        temp)
    break
